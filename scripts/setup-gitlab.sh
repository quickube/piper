...
# create a new user
user_id=$(curl --silent --location "${GITLAB_BASE_URL}/users" --header "${CONTENT_JSON_HEADER}" --header "${GITLAB_TOKEN_HEADER}" \
--data '{
...
group_id=$(curl --silent --location "${GITLAB_BASE_URL}/groups" --header "${CONTENT_JSON_HEADER}" --header "${GITLAB_TOKEN_HEADER}" \
--header "${IMPERSONATE_USER_HEADER}"  --data '{"name": "Pied Pipers", "path": "pied-pipers"}' | jq -r ".id")
sleep 3

# create a project on group namespace
reponse=$(curl --silent --location "${GITLAB_BASE_URL}/projects" --header "${CONTENT_JSON_HEADER}" --header "${GITLAB_TOKEN_HEADER}" \
--header "${IMPERSONATE_USER_HEADER}" --data "{\"name\":\"piper-e2e-test\", \"namespace_id\": \"${group_id}\"}")
sleep 3

# create a user personal access token
USER_TOKEN=$(curl --silent --location "${GITLAB_BASE_URL}/users/${user_id}/personal_access_tokens" --header "${CONTENT_JSON_HEADER}" --header "${GITLAB_TOKEN_HEADER}" \
